...
flowchart TB
    ...
    Web --> Control["control-plane-api\nregistry, governance, audit, monitoring"]
...
| --- | --- | --- | --- |
| `control-plane-api` | Tracks datasets, models, prompts, deployments, approvals, model cards, prompt cards, prediction events, drift snapshots, and audit events. | FastAPI plus PostgreSQL from Compose. | Azure Container Apps plus Azure Database for PostgreSQL or supplied `DATABASE_URL`. |
